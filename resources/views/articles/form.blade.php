<html>
<body>
<h3 class="page-title">{{isset($record) ? 'Edit' : 'New'}} Article</h3>

@if(isset($record))
    <form action="{{ route('articles.update',$record->id) }}" method="POST">
        @method('PUT')
        @else
            <form action="{{ route('articles.store') }}" method="POST" class="add-new-post">
                @endif
                @csrf
